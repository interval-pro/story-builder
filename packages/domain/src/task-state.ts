...
const BASE_TRANSITIONS: Record<TaskState, TaskState[]> = {
  ...
  // BLOCKED from both of these because implementation starts from both, and
  // starting it first checks for blocking conflicts with other active stories.
  // Without it the block threw: confirming answered 409 and did nothing, and
  // approving left the task approved with no job and no way forward.
  REVIEW_APPROVED: ['IMPLEMENTATION_QUEUED', 'HIGH_RISK_CONFIRMATION_REQUIRED', 'WAITING_FOR_TASK', 'BLOCKED'],
  HIGH_RISK_CONFIRMATION_REQUIRED: ['IMPLEMENTATION_QUEUED', 'REVIEW_FEEDBACK_RECEIVED', 'BLOCKED'],
